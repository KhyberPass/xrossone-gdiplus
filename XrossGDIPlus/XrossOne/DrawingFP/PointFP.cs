...
namespace XrossOne.DrawingFP
{
	public class PointFP
	{
		...
		{
			//Reset(MathFP.Mul(X, m.Sx) + MathFP.Mul(Y, m.Ry) + m.Tx, 
			//MathFP.Mul(Y, m.Sy) + MathFP.Mul(X, m.Rx) + m.Ty);
			int x1 = (int)(((long)X * m.Sx + (long)Y * m.Ry) >> SingleFP.DecimalBits) + m.Tx;
			int y1 = (int)(((long)Y * m.Sy + (long)X * m.Rx) >> SingleFP.DecimalBits) + m.Ty;
			X = x1;
			Y = y1;
			return this;
		}
