...
namespace XrossOne.DrawingFP
{
	...
	{
		internal const sbyte CMD_NOP		= 0;
		internal const sbyte CMD_MOVETO		= 1;
		internal const sbyte CMD_LINETO		= 2;
		internal const sbyte CMD_QCURVETO	= 3;
		internal const sbyte CMD_CCURVETO	= 4;
		internal const sbyte CMD_ARCTO		= 5;
		internal const sbyte CMD_CLOSE		= 6;
		...
		public static GraphicsPathFP CreateRoundRect(int ff_xmin, int ff_ymin, int ff_xmax, int ff_ymax, int ff_rx, int ff_ry)
		{
			...
			if (ff_rx != 0 && ff_ry != 0)
				path.AddPath(GraphicsPathFP.CreateArc(ff_xmax - ff_rx * 2, ff_ymin, ff_xmax, ff_ymin + ff_ry * 2, 0, -FF_PI / 2, false, false));
			path.AddLineTo(new PointFP(ff_xmax, ff_ymin + ff_ry));
			path.AddLineTo(new PointFP(ff_xmax, ff_ymax - ff_ry));
			if (ff_rx != 0 && ff_ry != 0)
				path.AddPath(GraphicsPathFP.CreateArc(ff_xmax - ff_rx * 2, ff_ymax - ff_ry * 2, ff_xmax, ff_ymax, FF_PI / 2, -FF_PI / 2, false, false));
			path.AddLineTo(new PointFP(ff_xmax - ff_rx, ff_ymax));
			path.AddLineTo(new PointFP(ff_xmin + ff_rx, ff_ymax));
			if (ff_rx != 0 && ff_ry != 0)
				path.AddPath(GraphicsPathFP.CreateArc(ff_xmin, ff_ymax - ff_ry * 2, ff_xmin + ff_rx * 2, ff_ymax, FF_PI, -FF_PI / 2, false, false));
			path.AddLineTo(new PointFP(ff_xmin, ff_ymax - ff_ry));
			path.AddLineTo(new PointFP(ff_xmin, ff_ymin + ff_ry));
			if (ff_rx != 0 && ff_ry != 0)
				path.AddPath(GraphicsPathFP.CreateArc(ff_xmin, ff_ymin, ff_xmin + ff_rx * 2, ff_ymin + ff_ry * 2, 3 * FF_PI / 2, -FF_PI / 2, false, false));
			path.AddClose();
			return path;
		}
		...
		public GraphicsPathFP(GraphicsPathFP from, MatrixFP matrix)
		{
			cmdsSize = from.cmdsSize;
			pntsSize = from.pntsSize;
			if (cmdsSize > 0)
			{
				...
				for (int i = 0; i < pntsSize; i++)
				{
					pnts[i] = new PointFP(from.pnts[i]).Transform(matrix);
				}
			}
		}
		...
		public static GraphicsPathFP  CreateArc(int ff_xmin, int ff_ymin, int ff_xmax, int ff_ymax, int ff_startangle, int ff_sweepangle, bool closed, bool standalone)
		{
			...
			int segments = MathFP.Round(MathFP.Div(24 * MathFP.Abs(ff_sweepangle), MathFP.PI)) >> SingleFP.DecimalBits;
			if (segments == 0) segments = 1;
			GraphicsPathFP path = new GraphicsPathFP();
			int ff_darg = ff_sweepangle / segments;
			int ff_arg = ff_startangle;
			int ff_cos = MathFP.Cos(ff_startangle);
			int ff_sin = MathFP.Sin(ff_startangle);
			...
			int ff_x = ff_xc + MathFP.Mul(ff_rx, ff_cos);
			int ff_y = ff_yc + MathFP.Mul(ff_ry, ff_sin);
			if (closed)
				path.AddLineTo(new PointFP(ff_x, ff_y));
			else
				if (standalone)
					path.AddMoveTo(new PointFP(ff_x, ff_y));
			...
			for (int i = 1; i <= segments; i++)
			{
				...
				ff_cos = MathFP.Cos(ff_arg);
				ff_sin = MathFP.Sin(ff_arg);
				ff_x = ff_xc + MathFP.Mul(ff_rx, ff_cos);
				ff_y = ff_yc + MathFP.Mul(ff_ry, ff_sin);
				if (MathFP.Abs(ff_lx - ff_x) + MathFP.Abs(ff_ly - ff_y) > SingleFP.One)
				{
					path.AddLineTo(new PointFP(ff_x, ff_y));
					ff_lx = ff_x;
					ff_ly = ff_y;
...
/*		public static GraphicsPathFP  CreateArc(int ff_xmin, int ff_ymin, int ff_xmax, int ff_ymax, int ff_startangle, int ff_sweepangle, bool closed, bool standalone)
		...
		*/
		...
		{
			if (pnts != null && pnts[pntsSize-1].Equals(control1))
			{
