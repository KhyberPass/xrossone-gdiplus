...
{	
	sealed class GraphicsPathOutline:GraphicsPathSketch
	{
		...
		public override void  MoveTo(PointFP point)
		{
			...
			lastPoint = currPoint = new PointFP(point);
			base.MoveTo(point);
		}
		...
		private void  AddLineCap(PointFP p1, PointFP p2, int lineCap)
		{
			...
			PointFP[] cap;
			if (lineCap == PenFP.LINECAP_TRIANGLE)
				cap = new PointFP[]{GraphicsPathFP.roundCap[3]};
			else
				cap = lineCap == PenFP.LINECAP_ROUND?GraphicsPathFP.roundCap:GraphicsPathFP.squareCap;
