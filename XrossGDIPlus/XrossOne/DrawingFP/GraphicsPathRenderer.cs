...
namespace XrossOne.DrawingFP
{
	public sealed class GraphicsPathRenderer:GraphicsPathSketch
	{
		...
		public GraphicsPathRenderer(IGraphicsBuffer buffer)
		{
			...
			this.buffer		= buffer;
			colors			= new int[buffer.Width];

			rawPixels		= ((SimpleGraphicsBuffer)buffer).buffer;
		...
		public void  DrawPath(GraphicsPathFP path, BrushFP style, int mode)
		{
			...
			if (transformMatrix != null)
				path = new GraphicsPathFP(path, transformMatrix);
			path.Visit(this);
		...
		public void  FinalizeBuffer(int color)
		{
			...
			for (int y = 0; y < Height; y++)
			{
				...
				{
					Color c = Color.FromArgb(rawPixels[x, y]);
					if (c.A != 0x00)
					{
						if (c.A != 0xFF)
						{
							rawPixels[x, y] = Color.FromArgb(
		...
		public override void  LineTo(PointFP point)
		{
			...
			Scanline(CurrentPoint.X, CurrentPoint.Y, point.X, point.Y);
...
*/
		public void  DrawBuffer()
		{
			isMonoColor = fillStyle.MonoColor;
			if (isMonoColor)
			{
				fillStyle.GetColorAt(0, 0, colors, 1);
				monoColor = colors[0];
			}
				...
				if ((newx != curx) || (newy != cury))
				{
					...
					if (alp != 0)
					{
						...
						if (alp != 0)
						{
							...
							{
								fillStyle.GetColorAt(curx, cury, colors, 1);
								color = colors[0];
							}
							color2 = ((((color >> 24) & 0xFF) * alp) >> 8) << 24 | color & 0xFFFFFF;
							color1 = rawPixels[curx, cury];
							{
								...
								rawPixels[curx, cury] = Ca << 24 | Cr << 16 | Cg << 8 | Cb;
							}
						}
					}
					}
		...
		private void  MergePixels(int x, int y, int count, int opacity)
		{
			...
			{
				fillStyle.GetColorAt(x, y, colors, count);
				for (int i = 0; i < count; i++)
					rawPixels[x + i, y] = Merge(rawPixels[x + i, y], colors[i]);
			}
		}
