...
namespace XrossOne.DrawingFP
{
	public class GradientBrushFP:BrushFP
	{
		...
		public override void GetColorAt(int x, int y, int[] colors, int count)
		{
			...
			PointFP p1 = null;
			if (len > 1) p1 = new PointFP(p.X + SingleFP.One * len, p.Y);
			if (finalMatrix != null)
			{
				p.Transform(finalMatrix);
				if (len > 1) p1.Transform(finalMatrix);
			}
			...
			if (type == LINEAR_GRADIENT)
			{
				...
				//if (v < 0) v = 0; else if (v > ff_length - 1) v = ff_length - 1;
				...
				float step = (float)ff_deltapos/len/SingleFP.One;
				float fpos = pos;
				for (int i = 0; i < len; i ++, fpos += step)
				{
					pos = (int)fpos;
					colors[i] = gradientColors[pos < 0?0:(pos > RATIO_MAX?RATIO_MAX:pos)];
				}
			}
			else
			{
				ff_currpos = MathFP.Div(PointFP.Distance(p.X, p.Y), ff_length >> 1);
				pos = ff_currpos >> SingleFP.DecimalBits - RATIO_BITS;
				colors[0] = gradientColors[pos < 0?0:(pos > RATIO_MAX?RATIO_MAX:pos)];
				if (len > 1) 
				{
					int xStep = (p1.X - p.X) / len;
					int yStep = (p1.Y - p.Y) / len;
					int xn = p.X + xStep;
					int yn = p.Y + yStep;
					for (int i = 1; i < len; i++, xn += xStep, yn += yStep)
					{
						ff_currpos = MathFP.Div(PointFP.Distance(xn, yn), ff_length >> 1);
						pos = ff_currpos >> SingleFP.DecimalBits - RATIO_BITS;
						colors[i] = gradientColors[pos < 0?0:(pos > RATIO_MAX?RATIO_MAX:pos)];
					}
				}
